  ...
  return (
    <div className="rounded-xl bg-white px-5 py-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <a href="/transactions" className="text-sm text-grey-500 hover:text-grey-900">View All</a>
      </div>
      <ul className="divide-y divide-grey-100">
        {transactions.map((tx) => (
          <li key={tx.id} className="flex items-center justify-between py-3">
            <span className="text-sm font-medium">{tx.name}</span>
            <span className={`text-sm font-bold ${tx.amount < 0 ? 'text-grey-900' : 'text-green'}`}>
