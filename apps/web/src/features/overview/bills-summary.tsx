...
export function BillsSummary() {
  ...
  const totalPaid = bills.filter((b) => b.amount < 0).reduce((s, b) => s + Math.abs(b.amount), 0)

  return (
    <div className="rounded-xl bg-white px-5 py-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-bold">Recurring Bills</h2>
        <a href="/bills" className="text-sm text-grey-500 hover:text-grey-900">See Details</a>
      </div>
      <dl className="space-y-3">
        <div className="flex justify-between">
          <dt className="text-sm text-grey-500">Paid Bills</dt>
          <dd className="text-sm font-bold text-green">{paidCount} (${totalPaid.toFixed(2)})</dd>
        </div>
        <div className="flex justify-between">
